...
// const aesDecrypt = function(data: any) {
//   const key = Buffer.from("testtesttesttest",'utf8');
//   const algorithm = 'aes-128-ecb';
// 	var cipher = crypto.createDecipheriv(algorithm, key, null);
...
export const wrapFsWithAsar = (fs: Record<string, any>) => {
  ...
  fs.readFile = function (pathArgument: string, options: any, callback: any) {
    ...
    let arrayBuffer: ArrayBuffer | string;
    try {
      arrayBuffer = archive.readSync(info.offset, info.size);
      if (info.encrypted === true) {
        arrayBuffer = asar.decodeBuffer(arrayBuffer, info.len);
      } 
    } catch (err) {
